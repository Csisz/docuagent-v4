...
const STAT_DEFS = [
  { key: "total",          label: "Total Invoices", icon: FileText,    accent: "blue"    },
  { key: "pending_review", label: "Review",         icon: Clock,       accent: "amber"   },
  { key: "verified",       label: "Verified",       icon: CheckCircle, accent: "blue"    },
  { key: "exported",       label: "Exported",       icon: Download,    accent: "emerald" },
  { key: "this_month",     label: "This Month",     icon: TrendingUp,  accent: "violet"  },
]
...
const STATUS_BAR = [
  { key: "pending_review", label: "Review",   color: "bg-amber-400"   },
  { key: "verified",       label: "Verified", color: "bg-blue-500"    },
  { key: "exported",       label: "Exported", color: "bg-emerald-500" },
]
...
export default function InvoiceStats() {
  ...
  if (isLoading) {
    return <div className="py-12 text-center text-[13px] text-muted-foreground">Loading…</div>
  }

  if (error || !stats) {
    return <div className="py-12 text-center text-[13px] text-red-500">Failed to load statistics</div>
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-[13px] font-semibold text-foreground">AI Avg. Confidence</h3>
        </div>
        ...
        <p className="text-[11px] text-muted-foreground mt-2">
          {(stats.avg_confidence ?? 0) >= 0.85
            ? "Excellent — most invoices can be processed automatically"
            : (stats.avg_confidence ?? 0) >= 0.65
            ? "Moderate — some invoices require manual review"
            : "Low — most invoices need manual verification"
          }
        </p>
      </div>
      ...
      {((stats.overdue ?? 0) > 0 || (stats.due_soon ?? 0) > 0) && (
        <div className="bg-card border border-border rounded-xl p-5">
          <h3 className="text-[13px] font-semibold text-foreground mb-3">Payment Deadlines</h3>
          <div className="flex gap-6 flex-wrap">
            {(stats.overdue ?? 0) > 0 && (
              <div className="flex items-center gap-2">
                <AlertTriangle size={14} className="text-red-500 flex-shrink-0" />
                <span className="text-[13px] font-semibold text-red-600">Overdue: {stats.overdue}</span>
              </div>
            )}
            {(stats.due_soon ?? 0) > 0 && (
              <div className="flex items-center gap-2">
                <Clock size={14} className="text-amber-500 flex-shrink-0" />
                <span className="text-[13px] font-semibold text-amber-600">Due within 3 days: {stats.due_soon}</span>
              </div>
            )}
          </div>
        </div>
      ...
      {stats.total > 0 && (
        <div className="bg-card border border-border rounded-xl p-5">
          <h3 className="text-[13px] font-semibold text-foreground mb-4">Status Breakdown</h3>

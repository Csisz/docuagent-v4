...
falling back to the mini model on the second attempt.
"""
import json
import logging
...
from core.ai_engine import _chat as ai_chat, MODEL_MINI, MODEL_SMART
from modules.email.agents.knowledge import KnowledgeContext
from modules.email.agents.intake import IntakeContext
...
async def classify(
    ...
    Retries once if JSON parse fails, falling back to mini model.
    ...
    for attempt in range(2):
        try:
            ...
            if attempt == 0:
                model = MODEL_MINI  # fallback to mini on retry

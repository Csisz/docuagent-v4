...
async def require_api_key(
    ...
    API key fallback: ha nincs JWT, az X-API-Key-t nezzu DB-bol.
    """
    if credentials:
        try:
    ...
    if x_api_key:
        # Legacy dashboard key bypass
        if DASHBOARD_API_KEY and x_api_key == DASHBOARD_API_KEY:
            return
        # Validate against tenant_api_keys table
...
async def get_current_user(
    ...
    """JWT auth → user payload tenant_id-vel. API key → tenant lookup DB-bol."""
    if credentials:
        try:
    ...
    if x_api_key:
        ...
        tenant_id = await get_tenant_from_api_key(x_api_key)
        if tenant_id:
            return {"tenant_id": tenant_id, "auth_type": "api_key"}
        raise HTTPException(status_code=401, detail="Érvénytelen API kulcs")
...
async def get_tenant_from_api_key(api_key: str) -> Optional[str]:
    ...
    try:
        ...
        row = await _db.fetchrow(
            "SELECT id, tenant_id, key_prefix FROM tenant_api_keys WHERE key_hash=$1 AND is_active=TRUE",
            key_hash,
        )
        if not row:
            return None
        ...
        try:
            await _db.execute(
                """INSERT INTO audit_log (tenant_id, action, resource_type, resource_id)
                   VALUES ($1, 'api_usage', 'api_key', $2)""",
                row["tenant_id"], row["key_prefix"],
            )
        except Exception:
            pass
        return tenant_id
